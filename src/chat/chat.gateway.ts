...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async getAllRoomsFromDbAndWebSocket() {
    ...
    const finalRooms = new Set([...roomsFromDb, ...this.rooms]);
    return finalRooms;
  }
  ...
  async handleConnection(client: Socket) {
    ...
    this.emitOldRooms(client, { room: null });
  ...
  handleMessageToRoom(
    ...
    this.logger.log(
      `client ${this.users.get(client.id)} trimite mesajul ${data.message} in room ${data.room}`,
    );
    this.server.to(data.room).emit('mesaj_nou', {
      mesaj: data.message,
  ...
  async handleCreateRoom(
    ...
    this.emitOldRooms(client, { room: data });
  }
  ...
  private async emitOldRooms(client: Socket, data: { room: string | null }) {
    const messages = await this.chatService.getMessagesForRoom(data.room);
